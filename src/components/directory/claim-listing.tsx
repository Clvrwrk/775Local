...
export function ClaimListingPanel({
  ...
  useEffect(() => {
    ...
    let active = true;
    setChecking(true);
    void getMyListingClaim({ data: { listingId } }).then((result) => {
      if (!active) return;
      setChecking(false);
  ...
  if (claim) {
    ...
    return (
      <div className="mt-4 rounded-[24px] border border-line bg-card p-5">
        <p className="text-xs font-medium uppercase tracking-[0.16em] text-sage">Claim status</p>
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setStatus("saving");
    setError("");
    const method = domainHint ? "business_domain" : kind;
