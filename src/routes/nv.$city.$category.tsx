  ...
  head: ({ loaderData, params }) => ({
    meta: loaderData
      ? [
          ...
          { name: "description", content: `Browse reviewed ${loaderData.category.name.toLowerCase()} listings in ${loaderData.city.name}, Nevada.` },
        ]
      : [],
    links: [{ rel: "canonical", href: `https://775directory.com/nv/${params.city}/${params.category}` }],
  }),
    ...
    <SiteShell>
      <section className="app-page px-4 py-10 sm:px-6 sm:py-14">
        ...
        <p className="mt-4 max-w-2xl text-base leading-7 text-ink-soft">
          {category.description} These entries appear after their core business information passes publication review.
        </p>
        <div className="mt-4 flex flex-wrap gap-3 text-sm">
          <Link to="/nv/$city" params={{ city: city.slug }} className="font-medium text-teal hover:underline">
            All of {city.name}
          </Link>
          <Link
            to="/categories/$slug"
        ...
        <p className="mt-10 border-t border-line pt-5 text-xs leading-5 text-muted">Listings are informational. Confirm service area, availability, pricing, licensing, and insurance directly with the business before spending money.</p>
      </section>
    </SiteShell>
  );
}
