...
export const Route = createFileRoute("/categories_/$slug")({
  ...
  head: ({ loaderData, params }) => {
    const robots = loaderData ? robotsForListingCount("category", loaderData.results.length) : null;
    return {
      meta: loaderData
        ? [
            { title: `${loaderData.category.name} in Reno | 775Directory` },
            {
              name: "description",
              content: `Browse reviewed ${loaderData.category.name.toLowerCase()} listings in Reno.`,
            },
            ...(robots ? [robots] : []),
          ]
        : [],
        ...
        <div className="mt-9 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {results.length ? (
            ...
            <p className="rounded-[24px] border border-line bg-card p-8 text-sm leading-6 text-muted sm:col-span-2 lg:col-span-3">
              No reviewed {category.name.toLowerCase()} listings are published yet. Try a town page
              or check back as more entries clear review.
            </p>
          )}
        </div>
      </section>
