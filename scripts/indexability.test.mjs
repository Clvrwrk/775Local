...
import { INDEXING_THRESHOLDS, robotsForListingCount } from "../src/lib/directory/indexability.mjs";
...
test("thin discovery pages remain noindex until reviewed listing thresholds are met", () => {
  ...
  assert.deepEqual(robotsForListingCount("city", 2), {
    name: "robots",
    content: "noindex, follow",
  });
  assert.equal(robotsForListingCount("city", 3), null);
  assert.deepEqual(robotsForListingCount("cityCategory", 4), {
    name: "robots",
    content: "noindex, follow",
  });
...
test("discovery routes apply the reviewed-listing indexability gate", async () => {
  const routes = await Promise.all(
    ["nv.$city.tsx", "nv_.$city.$category.tsx", "categories_.$slug.tsx"].map((name) =>
      readFile(new URL(`../src/routes/${name}`, import.meta.url), "utf8"),
    ),
  );
...
test("leaf discovery routes render independently instead of being hidden by parent pages", async () => {
  const routeTree = await readFile(new URL("../src/routeTree.gen.ts", import.meta.url), "utf8");
  assert.match(
    routeTree,
    /const CategoriesSlugRoute[\s\S]*?path: '\/categories\/\$slug',[\s\S]*?getParentRoute: \(\) => rootRouteImport/,
  );
  assert.match(
    routeTree,
    /const NvCityCategoryRoute[\s\S]*?path: '\/nv\/\$city\/\$category',[\s\S]*?getParentRoute: \(\) => rootRouteImport/,
  );
});
