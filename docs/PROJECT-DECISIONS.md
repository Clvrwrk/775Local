...
- The temporary Production API key was exposed in task input and cannot be revoked immediately through WorkOS; its earliest supported expiration was set for 2026-08-24 16:06 America/Los_Angeles. Its Production callback remains unchanged because changing a Production redirect is outside the Preview-only approval. Until the Vercel branch variables are rotated, the Preview application is not accepted even though Supabase already rejects the old issuer.
- Vercel credential rotation remains pending a fresh Vercel login and a direct, non-clipboard transfer of the isolated Staging secret. Copying the WorkOS environment-variable bundle to the system clipboard was rejected as an unnecessary secret-exposure path.
- Provider-level proof of the JWT claim template, login methods, session durations, and Operations-organization MFA is complete in isolated Staging. Direct Vercel secret rotation and an end-to-end authenticated-user flow are still required before CLE-106 can close.
