...
## 2026-09-07 — Listing-linked website intelligence ledger

- **Canonical account:** Maintain one private `listing_intelligence_accounts` row for every Business Listing with a web source, backed by normalized append-only captures, pages, fact sets, SEO audits, and raw artifacts. Do not store an entire website in one oversized mutable JSON row.
- **Full-capture definition:** “Full” means the entire accessible owned site under the recorded Firecrawl configuration, with provider pagination drained and no cap, failed page, or access blocker. Any limitation is retained and fails closed to `partial` or `blocked`. Third-party platforms are limited to the registered business landing page.
- **Privacy and provenance:** Extracted email, phone, address, service-area, service, and differentiator evidence stays private with page-level provenance. Public drafts must exclude private email/residential-address data and unsupported claims.
- **SEO audit:** DataForSEO OnPage audit runs are Listing-linked and preserve full task/status/summary/page/Lighthouse payloads, task IDs, configuration, page counts, limitations, and cost. Platform landing pages are not treated as owned websites for OnPage audits.
- **Content boundary:** Competitive top-service content is an Operator-reviewed candidate with a local-searcher decision tree, claim provenance, and uniqueness receipt. No automated capture, audit, or generation step can publish a Listing.
- **Spend and environment:** Planning is zero-provider and zero-write. Live Firecrawl/DataForSEO execution requires an explicit worst-case credit/dollar envelope and Preview-only target validation. Production migration/apply retains its separate exact approval and validation gate.
