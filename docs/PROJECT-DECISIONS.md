...
## 2026-08-26 — Enrichment archive and robust seed proposal boundary

- Preserve every collected Local775 SERP/crawl JSON or JSONL artifact in a byte-verifiable, content-addressed Supabase archive so later work reuses saved evidence and requests only missing, stale, or insufficient enhancements rather than automatically buying another full crawl.
- The shared database destination for this work is persistent Supabase Preview `dpxeldzunfxmjahgvjhm`; Production remains separately human-gated. Raw evidence and derived private contact facts are service-role-only. Operators may see normalized review proposals, while anonymous users see neither archive nor proposals.
- Build the strongest possible private `seed-profile-v1` proposal for each of the first 400 current SERP results from the evidence already collected. Keep facts traceable to source pages and preserve uncertainty, failures, and stale receipts.
- Website photos are source-linked, ranked review candidates only. Do not hotlink, copy into public media, assert usage rights, or publish a photo until a human verifies business identity, rights, accuracy, and suitability.
- Archive sync is not publication. It may not write canonical Listings, review decisions, media derivatives, participation, entitlements, billing, ranking, or publication state.
- Christopher Hussey explicitly approved the 88,279,559-byte manifest `88be81201a8dcf83ee88191a3d6f24d34c3616533931aadfd7b65b7462bebbf9`, including collected website/contact evidence, for Preview `dpxeldzunfxmjahgvjhm`. Snapshot `1` stores 496 raw artifacts, 400 private profile proposals, and 9,690 private photo-evidence candidates; its idempotent rerun inserted zero rows. No Production, canonical Listing, review, selection, photo publication, or provider effect was approved or performed.
