...
## 2026-08-26 — CLE-104 raw evidence archive and robust seed proposals prepared

- **Accounting:** [CAT-76](https://linear.app/cleverwork/issue/CAT-76/trailcle-104-build-local775-multi-provider-serp-enrichment-pipeline) and [CLE-104](https://linear.app/cleverwork/issue/CLE-104/launch-slice-seed-ingestion-and-reviewed-publication-set) record the user-requested raw Supabase archive, no-recrawl reuse rule, robust first-400 proposals, Preview-only boundary, and photo-review constraint. The durable disk receipt remains `/Users/chussey/Library/CloudStorage/Dropbox-AIA4/Cleverwork Main/CODEX/docs/linear/teams/CLE/2026-08-25-local775-serp-enrichment.md`.
- **Implementation:** branch `codex/cle-104-enrichment-archive` adds content-addressed raw snapshots, exact raw UTF-8 text plus parsed JSON, SHA-256/byte/path manifests, private normalized profile proposals, ranked source-linked website-photo candidates, service-role-only ingest RPCs, RLS, and append-only protections. The sync command is target-explicit, manifest-pinned, idempotent, and rejects Production.
- **Verified source snapshot:** manifest `88be81201a8dcf83ee88191a3d6f24d34c3616533931aadfd7b65b7462bebbf9` contains 496 JSON/JSONL artifacts and 88,279,559 bytes. The current result join produces exactly 400 `seed-profile-v1` proposals: 380 complete receipts, 6 crawl failures, 12 invalid/empty receipts, and 2 stale exact-URL receipts. It also produces 9,690 ranked photo URL candidates with page provenance. Zero filter-v3 candidates are accepted; no shortfall is padded.
- **Preview state:** Christopher Hussey explicitly approved loading the 88,279,559-byte raw archive, including collected website/contact evidence, into Supabase Preview `dpxeldzunfxmjahgvjhm`. Snapshot `1` reconciles manifest `88be81201a8dcf83ee88191a3d6f24d34c3616533931aadfd7b65b7462bebbf9` to 496 raw artifacts, 400 private profiles, 9,690 private photo-evidence candidates, and 88,279,559 bytes. The manifest-pinned rerun inserted zero artifact links and zero profiles. Supabase Production `hcfryjrajqftcnnbnybj` remains untouched.
- **Verification:** fresh migration replay passes; pgTAP passes 112/112; schema lint has no errors; all 110 Node tests, typecheck, ESLint, secret scan, diff check, and the Vercel production build pass. The first real-scale reconciliation exposed and then fixed a fan-out timeout in `enrichment_snapshot_status`; the forward fix uses independent aggregate CTEs, and the sync command now short-circuits a fully reconciled manifest before retransmitting 88.3 MB. Preview advisors report no warning/error findings; three informational RLS notices reflect intentionally policy-free private archive tables, and the remaining performance notices are unused-index observations on the new/quiet Preview database. No provider was called and no provider spend was added.
- **Publication boundary:** profile fields and website photos are proposals only. No canonical Listing, review outcome, media derivative, entitlement, ranking, billing, or publication state was written. Photo rights and suitability remain human-reviewed.

**Resume here:** keep the nightly automation on its existing bounded filter-v3 batch-01 revalidation. A later archive sync must dry-run and pin its new exact manifest; a database-only retry must use the status-first idempotent path and must not recrawl. Do not import to Production, accept filter-v2 proposals, or publish proposals.
