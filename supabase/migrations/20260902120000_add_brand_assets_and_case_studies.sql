...
--
-- Photo model: has_logo reports approved logo presence; upload/review commands
-- and any required-logo workflow remain separate. Encouraged: horizontal and vertical
...
-- Retain all existing media and references. Select one primary slot without
-- deleting duplicate uploads or clearing approved paths. Replacements remain
-- separate assets until a reviewed, recoverable media command selects them.
alter table app.media_assets add column is_brand_primary boolean not null default false;
with ranked as (
  select m.id, row_number() over (
    partition by m.listing_id, m.kind
    order by (lc.logo_media_id = m.id or lc.hero_media_id = m.id) desc nulls last,
      (m.status = 'approved') desc, m.created_at desc, m.id desc
  ) as rn
...
update app.media_assets m set is_brand_primary = true
from ranked where ranked.id = m.id and ranked.rn = 1;

create unique index media_assets_one_per_brand_slot
  on app.media_assets (listing_id, kind)
  where is_brand_primary
    and kind in ('logo', 'logo_horizontal', 'logo_vertical', 'favicon', 'owner_headshot')
...
comment on column app.media_assets.kind is
  'logo is encouraged per listing. logo_horizontal, logo_vertical, favicon, owner_headshot are single slots; storefront, vehicle_wrap, project, product, image, video repeat.';
...
using (
  client_consented and (status = 'published' or (status = 'archived' and is_featured))
  and exists (
...
) on app.case_studies to anon, authenticated;
grant select (case_study_id, media_id, role, caption, sort_order) on app.case_study_media to anon, authenticated;
grant select (id, listing_id, kind, public_path, media_type, caption, sort_order, status) on app.media_assets to anon, authenticated;
-- Existing table-wide media SELECT must not expose originals through the new public policy.
revoke select on app.media_assets from anon, authenticated;
grant select (id, listing_id, kind, public_path, media_type, caption, sort_order, status)
  on app.media_assets to anon, authenticated;
-- No owner case-study editing UI/command exists yet. Keep writes closed instead
-- of granting publication-capable CRUD that skips review, revision and audit.
revoke insert, update, delete on app.case_studies, app.case_study_media from anon, authenticated;
