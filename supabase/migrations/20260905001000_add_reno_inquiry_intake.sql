...
declare destination uuid; existing app.leads%rowtype; new_lead_id uuid;
begin
  if jsonb_typeof(requested_payload) is distinct from 'object'
    ...
    or requested_payload->>'consent' is distinct from 'true'
  ...
  insert into app.leads(listing_id,idempotency_key,resident_name,resident_email,resident_phone_e164,resident_postal_code,request_text,contact_consent_at,source_path,source_context,destination_id,abuse_key)
  values(requested_listing_id,requested_key,trim(requested_payload->>'name'),lower(requested_payload->>'email'),nullif(requested_payload->>'phone',''),requested_payload->>'zip',trim(requested_payload->>'message'),statement_timestamp(),'/biz/'||(select current_slug from app.business_listings where id=requested_listing_id),'reno-inquiry-v1',destination,requested_abuse_key) returning id into new_lead_id;
